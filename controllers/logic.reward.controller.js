...
// Import Config
const { EMAIL_REG, Gateway } = require("../config/constants");
...
module.exports.logicVerifyMemberMobile = async (card, mobile, code) => {
  try {
    ...
    let parallel = await Promise.all([
      ...
      cardModel.readMembershipCard("id", card, 1)
    ]);
    ...
    if (parallel[1].length === 0)
      return {
        success: false,
        data: [],
        msg: "Wrong memebership card"
      };

    // Read Membership Card Owner
    let cardRecord = await cardLinkCustomerModel.readCardOwner(
      "customer_information_id",
      parallel[1].id
    );
    ...
    if (
      cardRecord[0].customer_information_id !==
      parallel[0].customer_information_id
    )
      return {
        success: false,
        data: [],
    ...
    else if (parallel[0].length === 0 && cardRecord[1].length === 0) {
...
module.exports.logicKeepCustomerData = async (
  email,
  mobile,
  code,
  card,
...
module.exports.logicGetAllData = async (mobile, code) => {
  try {
    ...
    const list = await logicRewardQuestionList(
      customerParse[0].customer_information_id
    );

    // Logic Coupon List
    const couponList = logicCouponList(
      customerParse[0].customer_information_id
    );
    ...
    return [
      {
        ...
        value: 0,
        detail:
          "This is best strategy to get free stuff on Amazon, as fast as possible, and with the least amount of effort required too. All you need to do is sign up for an account at the website and then click to view and then “buy” the product"
      },
      {
...
const logicRewardQuestionList = async customerId => {
  try {
    ...
    let questionParse = await rewardQuestion.readRewardQuestionList(
      "customer_reward_questions.reward_question_id, customer_reward_questions.reward_question, customer_reward_questions.input_id, customer_reward_questions.reward_point, input_types.input_name",
      1
    );
...
const creatRewardQuestionJson = async (json, customerId) => {
  try {
    const jsonArray = json.map(async list => {
      ...
      let rewardResponseParse = await rewardResponse.readRewardResponse(
        "*",
...
module.exports.logicRewardResponse = async (mobile, code, json) => {
  try {
    ...
    iterateRewardResponse(
      customerRecord[0].customer_information_id,
      customerRecord[0].reward_point,
      json
    );
...
const iterateRewardResponse = async (id, rewardPoint, json) => {
  try {
    ...
    const promises = json.map(async (reward, index) => {
      ...
      let questionParse = await rewardQuestion.readRewardQuestion(
        "*",
        parseInt(reward.question_id, 10),
        1
      );
      ...
      // Read All Reward Response by Question Id and Customer Id
      let rewardParse = await rewardResponse.readRewardResponse(
        "*",
        reward.question_id,
        id,
        1
      ...
      if (questionParse.length !== 0) {
        if (questionParse[0].input_id === 1) {
          // Radio
          if (reward.option.length === 1) {
            if (rewardParse.length === 0) {
              // Keep Question Reward Response
              rewardResponse.keepRewardResponse(
                parseInt(reward.question_id, 10),
                parseInt(reward.option[0], 10),
                id,
                undefined,
                1
              );
              ...
              if (
                parseInt(reward.option[0], 10) !==
                rewardParse[0].reward_option_id
              ) {
                // Update Reward Response
                await rewardResponse.updateRewardResponse(
                  rewardParse[0].question_response_id,
                  0
                );
        ...
        } else if (questionParse[0].input_id === 2) {
          // Checkbox
          if (rewardParse.length === 0) {
            reward.option.map(x => {
              // Keep Question Reward Response
            ...
            object.softDelete.map(x => {
              ...
              rewardResponse.updateResponseByOption(
                parseInt(reward.question_id, 10),
                parseInt(x, 10),
                0
              );
        ...
        } else if (questionParse[0].input_id === 3) {
          // 5 Star
        ...
        } else if (questionParse[0].input_id === 4) {
          // 10 Star
