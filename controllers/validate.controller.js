...
// Validate Stocks Parameter
module.exports.validateStoreStock = json => {
  try {
    for (let i = 0; i < json.length; i++) {
      if (
        !json[i].hasOwnProperty("barcode") ||
...
module.exports.validateStockLog = (json, actor) => {
  try {
    for (let i = 0; i < json.length; i++) {
      if (actor === "STORE") {
        if (
          ...
          return {
            success: false,
            msg: "Missing error stocks log user key parameter"
          };
      }
      if (
        !json[i].hasOwnProperty("barcode") ||
